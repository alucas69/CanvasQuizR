#' Write a multiple answer question to the quiz file
#'
#' This function writes an multiple answer question to the quiz file.
#' @param filestream is the link to the open file where the question can be saved.
#' @param key is a string holding the Canvas identifier for the quiz.
#' @param qkey is a string holding the Canvas identifier for the question
#' @param qname is a string holding the Canvas name for the question
#' @param intro_text is a vector of strings, holding the paragraphs of the quiz text.
#' @param answer_text is a vector of strings, holding the answers.
#' @param answer_correct is a vector of booleans, holding which answer(s) are true. Put the correct ones at the top.
#' @param answer_counter is an integer holding the 5 digit identifier of the answers across all questions
#' @keywords multiple answer question for canvas quiz
#' @export answer_counter holds the updated value of the counter after the current question
#' @examples
#' MAquestion(filehandler, "ckqkek38", "ca,aacooq", "q1", c("this is", "test"), c("yes", "no"), c(TRUE, FALSE), 25)

MAquestion <- function(fileConn, key,
                       qkey, qname,
                       intro_text,
                       answer_text,
                       answer_correct,
                       answer_counter) {

  #######################################
  #######################################
  ## Start question
  #######################################
  #######################################
  iNumberOfAnswers <- length(answer_text)
  vAnswerIdentifiers <- paste(answer_counter + (1 : iNumberOfAnswers))
  WLprint(fileConn, sprintf("          <item ident=\"%s\" title=\"%s\">", qkey, qname))
  WLprint(fileConn, "
            <itemmetadata>
              <qtimetadata>
                <qtimetadatafield>
                  <fieldlabel>question_type</fieldlabel>
                  <fieldentry>multiple_answers_question</fieldentry>
                </qtimetadatafield>
                <qtimetadatafield>
                  <fieldlabel>points_possible</fieldlabel>
                  <fieldentry>1.0</fieldentry>
                </qtimetadatafield>
                <qtimetadatafield>
                  <fieldlabel>original_answer_ids</fieldlabel>
  ")
  WLprint(fileConn, sprintf("                  <fieldentry>%s</fieldentry>", paste(vAnswerIdentifiers, collapse = ",") ))
  WLprint(fileConn, "
                  </qtimetadatafield>
                  <qtimetadatafield>
                    <fieldlabel>assessment_question_identifierref</fieldlabel>
  ")
  WLprint(fileConn, sprintf("                    <fieldentry>%s</fieldentry>", paste(qkey, "a", sep = "") ))
  WLprint(fileConn, "
                    </qtimetadatafield>
                  </qtimetadata>
                </itemmetadata>
                <presentation>
                  <material>
                    <mattext texttype=\"text/html\">&lt;div&gt;
  ")
  ######################################
  ## here comes the question
  ######################################
  for (i1 in 1:length(intro_text)) {
    WLprint(fileConn, sprintf("                    &lt;p&gt;%s&lt;/p&gt;", intro_text[i1]))
  }
  WLprint(fileConn, "
                    &lt;/div&gt;</mattext>
                  </material>
  ")
  ######################################
  ## here come the answer type and answers
  ######################################
  WLprint(fileConn, "
                  <response_lid ident=\"response1\" rcardinality=\"Multiple\">
                    <render_choice>
  ")
  for (i1 in 1:iNumberOfAnswers) {
    WLprint(fileConn, sprintf("                    <response_label ident=\"%s\">", vAnswerIdentifiers[i1]))
    WLprint(fileConn, "                      <material>")
    WLprint(fileConn, sprintf("                        <mattext texttype=\"text/plain\">%s</mattext>", answer_text[i1] ))
    WLprint(fileConn, "                      </material>")
    WLprint(fileConn, "                    </response_label>")
  }
  WLprint(fileConn, "
                    </render_choice>
                  </response_lid>
                </presentation>
                <resprocessing>
                <outcomes>
                  <decvar maxvalue=\"100\" minvalue=\"0\" varname=\"SCORE\" vartype=\"Decimal\"/>
                </outcomes>
                <respcondition continue=\"No\">
                  <conditionvar>
                    <and>
  ")
  ######################################
  ## here come the correct answer indicators
  ######################################
  for (i1 in 1:iNumberOfAnswers) {
    if (answer_correct[i1] != TRUE) WLprint(fileConn, "                      <not>")
    WLprint(fileConn, sprintf("                        <varequal respident=\"response1\">%s</varequal>", vAnswerIdentifiers[i1]))
    if (answer_correct[i1] != TRUE) WLprint(fileConn, "                      </not>")
  }
  WLprint(fileConn, "
                    </and>
                  </conditionvar>
                  <setvar action=\"Set\" varname=\"SCORE\">100</setvar>
                </respcondition>
              </resprocessing>
            </item>
  ")
  WLprint(fileConn, "")

  #######################################
  #######################################
  ## return the updated counter for
  ## answer identification
  #######################################
  #######################################
  return(answer_counter + iNumberOfAnswers)
}
