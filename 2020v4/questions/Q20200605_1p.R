Q20200605_1p <- function(QuizFile, Counters, NumberOfVariations) {
  ...
  for (i1 in 1:NumberOfVariations) {
    ...
    QuestionText <- c(
      ...
      $H_0: \\mu_1 - \\mu_2 = %1.1f$ at $\\alpha = %4.2f$. Compute the value of the 
      usual test statistic to test this hypothesis, using %d decimal places.",  
    ...
    Answers <- (dDiff - dDiffNull) / dSdDiff1 + 0.002 * matrix(c(0,-1,1), nrow = 1)
    # with unequal variances
    Answers <- rbind(Answers, 
                     (dDiff - dDiffNull) / sqrt((sd(Sample$Sample1)^2 + sd(Sample$Sample2)^2) / iN) 
                     + 0.002 * matrix(c(0,-1,1), nrow = 1))
    #
    # pose the question
    #
