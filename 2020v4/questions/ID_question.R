...
Q_Integrity <- function(sEmailaddress) {

  sText <- c(
    "<strong><big>Welcome to the BSTAT written exam.</big></strong>",
    "You are looking at the version for normal time students (so 120 + 5 minutes for the selfie). If you are entitled to <strong>extra time</strong>, close the current quiz now and go to the extra time exam quiz.",
#    "You are looking at the version for extra time students (so 150 + 5 minutes for the selfie). If you are <strong>not entitled</strong> to extra time, close the current quiz now and go to the normal time exam quiz.",
  ...
  vAnswers <- c(
    ...
    "I will make this exam by myself without any outside help for the entire duration of the exam."
  )
...
Q_IntegrityExit <- function() {
  ...
  vAnswers <- c(
    ...
    "I made this exam by myself without any outside help for the entire duration of the exam."
  )
  vOrder <- sample(1:length(vAnswers), length(vAnswers))
  vCorrect <- c(FALSE, FALSE, TRUE)[vOrder]
  vAnswers <- vAnswers[vOrder]
