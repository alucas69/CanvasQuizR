Q20200629_1x <- function(QuizFile, Counters, NumberOfVariations) {
  ...
  for (i1 in 1:NumberOfVariations) {
    ...
    QuestionText <- c(
      ...
      the tested person indeed does not have the disease).", dFreq, 100-dAlpha, 
              dBeta, 100-dBeta),
      sprintf("Compute the probability that a randomly chosen person will test positive for the 
              disease using %d decimal places.", iDigits),
